...
volatile timer_ticks_t timer_InterruptCount;
 ...
 * 返回值   :无
 ******************************************************************************/
void timer_us(u32 nus)
{
  SysTick->LOAD=nus*fac_us;           //时间加载,将要定时的微妙数直接进行定时
  SysTick->VAL=0x00;                  //清空计数器
  SysTick->CTRL|=SysTick_CTRL_ENABLE_Msk ;  //开始倒数

  timer_InterruptCount = 1;                   //因为直接定时所需时间,所以中断一次就可以了

  // Busy wait until the SysTick decrements the counter to zero.
  while (timer_InterruptCount != 0u)
  ;
 ...
 * 返回值   :无
 ******************************************************************************/
void timer_ms(u32 nms)
{
  SysTick->LOAD=(u32)fac_ms;
  SysTick->VAL =0x00;             //清空计数器
  SysTick->CTRL|=SysTick_CTRL_ENABLE_Msk ;  //开始倒数

  timer_InterruptCount = nms;  //中断的次数
