...
u8 rtcInit(void) {
	...
	if (BKP_ReadBackupRegister(BKP_DR1) != 0x5050){
		...
		RTC_WaitForLastTask();	//等待最近一次对RTC寄存器的写操作完成
		rtcSet(2015, 1, 14, 17, 42, 55);  //设置时间
...
void RTC_IRQHandler(void) {
	if (RTC_GetITStatus(RTC_IT_SEC) != RESET) //秒钟中断
		...
		printf("Alarm Time:%d-%d-%d %d:%d:%d\n", calendar.w_year,
				calendar.w_month, calendar.w_date, calendar.hour, calendar.min,
				calendar.sec); //输出闹铃时间
 ...
 * 返回值     ：0,成功;其他:错误代码.
 *---------------------------------------------------------------------------*/
u8 rtcSet(u16 syear, u8 smon, u8 sday, u8 hour, u8 min, u8 sec) {
	u16 t;
	u32 seccount = 0;
...
u8 rtcGet(void) {
	...
	calendar.week = rtcGetWeek(calendar.w_year, calendar.w_month,
			calendar.w_date); 	//获取星期
	return 0;
}

/*-----------------------------------------------------------------------------
 * 函数名称 ：rtcGetWeek
 * 函数介绍 ： 获得现在是星期几，功能描述:输入公历日期得到星期(只允许1901-2099年)
